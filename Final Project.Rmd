...
## Pooled regression
```{r}
emissions.pool <- plm(CO2.Emissions ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + gdp , data = cleaned_data, model = "pooling") 
summary(fatality.pool, vcov = vcovHC) #clustered SDs
```

## Fixed Individual Effects Regression
```{r}
emissions.fe <- plm(CO2.Emissions ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp+ gdp , data = cleaned_data, model = "within", effect="individual")
summary(emissions.fe, vcov = vcovHC)
```
## Fixed Time Effects Regression
```{r}
emissions.te <- plm(CO2.Emissions ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + gdp , data = cleaned_data, model = "within", effect="time")
summary(emissions.te, vcov = vcovHC)
```
## Fixed Individual and Time Effects Regression
```{r}
#fte code
emissions.fte <- plm(CO2.Emissions ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + gdp , data = cleaned_data,
