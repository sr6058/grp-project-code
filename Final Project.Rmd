...
setwd("/Users/ameyabhatnagar/Desktop/Econometrics/Project")
library(tidyverse)
library(lmtest)
library(stargazer)
library(dplyr)
...
Combined_Dataset_Econometrics <- read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/Combined Dataset - Econometrics.csv", na.strings = "")
lowcarb<-read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/transformed_trade_low_carbon_tech_corrected.csv", na.strings="")
renewable<-read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/transformed_renewable_energy.csv", na.strings="")
env_tax<-read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/transformed_environmental_taxes.csv", na.strings="")
env_exp<-read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/transformed_environmental_protection_expenditures_corrected.csv", na.strings="")
surface_temp<-read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/transformed_annual_surface_temp.csv", na.strings="")
gdp_per_capita <- read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/GDP_per_capita.csv", na.strings = "")
land_cover <- read.csv("/Users/ameyabhatnagar/Desktop/Econometrics/Project/Land_Cover_Accounts-2.csv", na.strings = "") 
land_cover <- land_cover[order(land_cover$Country), ]
```

## **Data Merging**

```{r}
final_set <- inner_join(lowcarb, renewable,by=c('Year',
                            'ISO3')) %>% 
  inner_join(env_tax,by=c('Year','ISO3')) %>%  
  inner_join(env_exp, by=c('Year',
                            'ISO3'))  %>%  
  inner_join(surface_temp, by=c('Year',
                            'ISO3')) %>%
  inner_join(Combined_Dataset_Econometrics, by = c('Year', 'ISO3')) %>%
  inner_join(gdp_per_capita, by = c('Year', 'ISO3')) %>%
  inner_join(land_cover, by = c('Year', 'ISO3'))


  
final_set <- subset(final_set, select = -c(Country.x, Country.y, Country.x.x, Country.y.y, Country.Name))
```
...
As can be inferred from the above plots, the variables $env.tax$, $lowcarbon$, $renewable.energy$,and $env.exp$ are not distributed normally and therefore have to be transformed to their log values that seem to be more normally distributed. However,  $surface.temp$ appears to be normally distributed and therefore can be left as is.
```{r}
#transformation code
```
