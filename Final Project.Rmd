...
final_set <- final_set %>% 
    rename(lowcarbon = Trade.in.Low.Carbon.Technology.Index, renewable.energy = Renewable.Energy.Capacity..MW., env.tax = Environmental.Taxes, 
           env.exp = Environmental.Protection.Expenditures, surface.temp=Annual.Surface.Temperature.Change, gdp=Value)
```
...
## Pooled regression
```{r}
emissions.pool <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + land_cover + gdp , data = final_set, model = "pooling") 
summary(fatality.pool, vcov = vcovHC) #clustered SDs
```
## Fixed Individual Effects Regression
```{r}
emissions.fe <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + land_cover + gdp , data = final_set, model = "within", effect="individual")
summary(emissions.fe, vcov = vcovHC)
```
## Fixed Time Effects Regression
```{r}
emissions.te <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + land_cover + gdp , data = final_set, model = "within", effect="time")
summary(emissions.te, vcov = vcovHC)
```
## Fixed Individual and Time Effects Regression
```{r}
#fte code
emissions.fte <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + land_cover + gdp , data = final_set,
