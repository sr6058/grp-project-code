...
gdppercapita<-read.csv("~/Documents/nyu/Final Assignment/GDP_per_capita.csv", na.strings = "")
```
...
## Pooled regression
```{r}
emissions.pool <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + gdp , data = final_set, model = "pooling") 
summary(fatality.pool, vcov = vcovHC) #clustered SDs
```
## Fixed Individual Effects Regression
```{r}
emissions.fe <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp+ gdp , data = final_set, model = "within", effect="individual")
summary(emissions.fe, vcov = vcovHC)
```
## Fixed Time Effects Regression
```{r}
emissions.te <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + gdp , data = final_set, model = "within", effect="time")
summary(emissions.te, vcov = vcovHC)
```
## Fixed Individual and Time Effects Regression
```{r}
#fte code
emissions.fte <- plm(CO2 ~ log.env.tax + log.lowcarbon + log.renewable.energy + log.env.exp + gdp , data = final_set,
